...
for d in Dirs:
    if os.path.isdir(os.path.join(root, d, 'Handler')):
        for f in os.listdir("%s/Handler" % d):
            if not f.startswith("__") and not os.path.isdir(os.path.join("%s/Handler" % d,f)) and not f.startswith("functions"):
                f = f.split(".")[0]
                exec ("from %s.Handler import %s" % (d,f))
