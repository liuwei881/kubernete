...
def nginxstart(projectname):
    ...
    result = local.cmd("docker",["cmd.run",],[["/usr/bin/kubectl create -f {0}/Kubernetes/Handler/projson/projects/{1}.json && /usr/bin/kubectl create -f {0}/Kubernetes/Handler/projson/projects/{1}-service.json".format(dir,projectname)]])
    return result
...
def appstart(appname):
    '''app 启动'''
    local = salt.client.LocalClient()
    result = local.cmd("docker",["cmd.run",],[["/usr/bin/kubectl create -f {0}/Kubernetes/Handler/projson/apps/{1}.json && /usr/bin/kubectl create -f {0}/Kubernetes/Handler/projson/apps/{1}-service.json".format(dir,appname)]])
    return result
